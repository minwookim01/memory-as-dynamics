"""Run RPM inference on the LaSOT-ext test set, split into chunks.

Launch one process per chunk to parallelize across GPUs/processes, e.g.:
    for i in 0 1 2 3; do
        CUDA_VISIBLE_DEVICES=$i python scripts/main_inference_chunk_lasot_ext.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description="RPM chunked inference on LaSOT-ext")
    parser.add_argument("--data_root", required=True,
                        help="LaSOT-ext root containing <video>/img and groundtruth.txt")
    parser.add_argument("--testing_set", default=None,
                        help="testing_set.txt (default: <data_root>/testing_set.txt)")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--config", default="configs/rpm/lasotext/sam2.1_hiera_b+.yaml")
    parser.add_argument("--output", default="results/lasot_ext")
