...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="index of the current chunk to process (0-based)",
    )
    parser.add_argument(
        "--num_chunks",
        type=int,
        default=1,
        help="total number of chunks to split videos into",
    )
    args = parser.parse_args()

    if args.num_chunks <= 0:
        raise ValueError(f"`num_chunks` must be >= 1, but got {args.num_chunks}.")
    if args.chunk_idx < 0 or args.chunk_idx >= args.num_chunks:
        raise ValueError(
            f"`chunk_idx` must be in [0, {args.num_chunks - 1}], but got {args.chunk_idx}."
        )
    ...
    chunked_video_names = video_names[args.chunk_idx :: args.num_chunks]

    print(
        f"running VOS prediction on chunk {args.chunk_idx + 1}/{args.num_chunks} "
        f"with {len(chunked_video_names)} videos:\n{chunked_video_names}"
    )

    for n_video, video_name in enumerate(chunked_video_names):
        print(
            f"\n{n_video + 1}/{len(chunked_video_names)} - "
            f"running on {video_name} (chunk {args.chunk_idx + 1}/{args.num_chunks})"
        )
        if not args.track_object_appearing_later_in_video:
            vos_inference(
                predictor=predictor,
    ...
    print(
        f"completed VOS prediction on chunk {args.chunk_idx + 1}/{args.num_chunks} "
        f"({len(chunked_video_names)} videos) -- "
        f"output masks saved to {args.output_mask_dir}"
    )
