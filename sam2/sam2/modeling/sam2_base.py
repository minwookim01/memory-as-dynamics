...
class SAM2Base(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        if tracklet_logit is not None:
            sam_mask_prompt = tracklet_logit
        ...
        if multimask_output and self.kf_mode:
            ...
            if self.rvcot_mem_selection and self.rvcot_mem_selection_method == 'default':
                # 'default' distractor-aware memory selection compares mask
                # bounding-box IoUs via the VOT toolkit. Imported lazily so the
                # heavy `vot` package is only required when this method is used.
                ...
                if frame_idx > 2:  # Ensure we have previous frames to evaluate
                    for i in range(frame_idx - 2, 1, -1):  # Iterate backwards through all previous frames
                        ...
                        if i in output_dict["non_cond_frame_outputs"]:
                            prev_out = output_dict["non_cond_frame_outputs"][i]
                        elif i in output_dict["cond_frame_outputs"]:
                            prev_out = output_dict["cond_frame_outputs"][i]
                        else:
                            prev_out = None

                        iou_score = prev_out.get("best_iou_score", None) if prev_out is not None else None
