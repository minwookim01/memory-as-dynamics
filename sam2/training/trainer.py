...
class Trainer:
    ...
    def _step(
        ...
        outputs, tracklets = model(batch)
        targets = batch.masks
        batch_size = len(batch.img_batch)

        key = batch.dict_key  # key for dataset
        loss = self.loss[key](outputs, targets, tracklets)
    ...
    def _setup_components(self):
        ...
        def freeze_all_params(model):
            ...
            print("[Freeze] All SAM2 parameters have been frozen.")

        freeze_all_params(self.model)

        #-------------------------unfreeze Future Module 
        for name, p in self.model.future_predictor.named_parameters():
            p.requires_grad = True

        print("[Unfreeze] Only future predictor is trainable.")
        '''
